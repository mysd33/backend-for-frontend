...
#logをjson形式にする場合
docker run -d -p 8081:8081 --name samplebff --env ENV_TYPE=dev,log_container --env BACKEND_URL=http://localhost:8080 --add-host=localhost:(ローカルPCのプライベートIP) XXXXXXXXXXXX.dkr.ecr.ap-northeast-1.amazonaws.com/mynavi-sample-ecs-bff:latest
```
* ECRプッシュ
```sh
